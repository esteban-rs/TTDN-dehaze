...
class TrainSet_(Dataset):
    def __init__(self, args, transform = transforms.Compose([RandomFlip(), RandomRotate(), ToTensor()])):
        ...
        # Take images from pix2pix
        self.lrup_list = [os.path.join(args.dataset_ref_dir, name.split('/')[-1]) for name in 
            self.lr_list]
    ...
    def __getitem__(self, idx):
        ...
        LR_sr  = imread(self.lrup_list[idx])
        LR_sr  = np.array(Image.fromarray(LR_sr).resize((self.args.img_size,self.args.img_size), Image.BICUBIC))
