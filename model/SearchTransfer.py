...
class SearchTransfer(nn.Module):
    ...
    def Search(self, lr_lv3, refsr_lv3, ref_lv1, ref_lv2, ref_lv3): 
        kernels = [[self.unfold_kernel_size_lv3, self.padding_lv3]]
        
        cor_sum = 0
        for i, k in enumerate(kernels) :
            
            ## ---- LOW RESOLUTION SPACE ----
            Q  = F.unfold(lr_lv3[2], kernel_size  = k[0], 
                                          padding = k[1], 
                                           stride = k[1])

            K = F.unfold(refsr_lv3[2], kernel_size = k[0], 
                                          padding  = k[1], 
                                           stride  = k[1])
            # Normalize patches
            K = K.permute(0, 2, 1)         # [N, C*k*k,  H*W] -> [N, H*W, C*k*k]
            K = F.normalize(K, dim = 2)    # [N, H*W, C*k*k] 
            Q = F.normalize(Q, dim = 1)    # [N, C*k*k, Hr*Wr]
            R = torch.bmm(K, Q)            # [N, H*W, Hr*Wr]
            
            cor_sum = cor_sum + R
            
        _, H = torch.max(cor_sum, dim = 1) # [N, Hr*Wr]

        # select normalized patches in low-resolution space
        K_ = self.bis(K.permute(0, 2, 1) , 2, H)  # [N, Hr*Wr, C*k*k] -> [N, C*k*k, H*W]
